...
static const char *polymorphic_string_as_utf8(id string) {
  if (string == nil)
    return "(nil)";
  NSString *characters;
...
static uint32_t vk_code_to_functional_key_code(uint8_t key_code) { // {{{
   ...
   */
  case 0x35:
    return GLFW_FKEY_ESCAPE;
  case 0x24:
    return GLFW_FKEY_ENTER;
  case 0x30:
    return GLFW_FKEY_TAB;
  case 0x33:
    return GLFW_FKEY_BACKSPACE;
  case 0x72:
    return GLFW_FKEY_INSERT;
  case 0x75:
    return GLFW_FKEY_DELETE;
  case 0x7b:
    return GLFW_FKEY_LEFT;
  case 0x7c:
    return GLFW_FKEY_RIGHT;
  case 0x7e:
    return GLFW_FKEY_UP;
  case 0x7d:
    return GLFW_FKEY_DOWN;
  case 0x74:
    return GLFW_FKEY_PAGE_UP;
  case 0x79:
    return GLFW_FKEY_PAGE_DOWN;
  case 0x73:
    return GLFW_FKEY_HOME;
  case 0x77:
    return GLFW_FKEY_END;
  case 0x39:
    return GLFW_FKEY_CAPS_LOCK;
  case 0x47:
    return GLFW_FKEY_NUM_LOCK;
  case 0x6e:
    return GLFW_FKEY_MENU;
  case 0x7a:
    return GLFW_FKEY_F1;
  case 0x78:
    return GLFW_FKEY_F2;
  ...
  case 0x65:
    return GLFW_FKEY_F9;
  case 0x6d:
    return GLFW_FKEY_F10;
  case 0x67:
    return GLFW_FKEY_F11;
  case 0x6f:
    return GLFW_FKEY_F12;
  case 0x69:
    return GLFW_FKEY_F13;
  case 0x6b:
    return GLFW_FKEY_F14;
  case 0x71:
    return GLFW_FKEY_F15;
  case 0x6a:
    return GLFW_FKEY_F16;
  case 0x40:
    return GLFW_FKEY_F17;
  case 0x4f:
    return GLFW_FKEY_F18;
  case 0x50:
    return GLFW_FKEY_F19;
  case 0x5a:
    return GLFW_FKEY_F20;
  case 0x52:
    return GLFW_FKEY_KP_0;
  case 0x53:
    return GLFW_FKEY_KP_1;
  case 0x54:
    return GLFW_FKEY_KP_2;
  case 0x55:
    return GLFW_FKEY_KP_3;
  case 0x56:
    return GLFW_FKEY_KP_4;
  case 0x57:
    return GLFW_FKEY_KP_5;
  case 0x58:
    return GLFW_FKEY_KP_6;
  case 0x59:
    return GLFW_FKEY_KP_7;
  case 0x5b:
    return GLFW_FKEY_KP_8;
  case 0x5c:
    return GLFW_FKEY_KP_9;
  case 0x41:
    return GLFW_FKEY_KP_DECIMAL;
  case 0x4b:
    return GLFW_FKEY_KP_DIVIDE;
  case 0x43:
    return GLFW_FKEY_KP_MULTIPLY;
  case 0x4e:
    return GLFW_FKEY_KP_SUBTRACT;
  case 0x45:
    return GLFW_FKEY_KP_ADD;
  case 0x4c:
    return GLFW_FKEY_KP_ENTER;
  case 0x51:
    return GLFW_FKEY_KP_EQUAL;
  case 0x38:
    return GLFW_FKEY_LEFT_SHIFT;
  case 0x3b:
    return GLFW_FKEY_LEFT_CONTROL;
  case 0x3a:
    return GLFW_FKEY_LEFT_ALT;
  case 0x37:
    return GLFW_FKEY_LEFT_SUPER;
  case 0x3c:
    return GLFW_FKEY_RIGHT_SHIFT;
  case 0x3e:
    return GLFW_FKEY_RIGHT_CONTROL;
  case 0x3d:
    return GLFW_FKEY_RIGHT_ALT;
  case 0x36:
...
static uint32_t vk_code_to_unicode(uint8_t key_code) { // {{{
  switch (key_code) {
...
static uint32_t mac_ucode_to_functional(uint32_t key_code) { // {{{
  switch (key_code) {
  /* start macu to functional (auto generated by gen-key-constants.py do not
   * edit) */
  ...
  case NSScrollLockFunctionKey:
    return GLFW_FKEY_SCROLL_LOCK;
  case NSClearLineFunctionKey:
    return GLFW_FKEY_NUM_LOCK;
  case NSPrintScreenFunctionKey:
    return GLFW_FKEY_PRINT_SCREEN;
  case NSPauseFunctionKey:
    return GLFW_FKEY_PAUSE;
  ...
  case NSF21FunctionKey:
    return GLFW_FKEY_F21;
  case NSF22FunctionKey:
    return GLFW_FKEY_F22;
  case NSF23FunctionKey:
    return GLFW_FKEY_F23;
  case NSF24FunctionKey:
    return GLFW_FKEY_F24;
  case NSF25FunctionKey:
    return GLFW_FKEY_F25;
  case NSF26FunctionKey:
    return GLFW_FKEY_F26;
  case NSF27FunctionKey:
    return GLFW_FKEY_F27;
  case NSF28FunctionKey:
    return GLFW_FKEY_F28;
  case NSF29FunctionKey:
    return GLFW_FKEY_F29;
  case NSF30FunctionKey:
    return GLFW_FKEY_F30;
  case NSF31FunctionKey:
    return GLFW_FKEY_F31;
  case NSF32FunctionKey:
    return GLFW_FKEY_F32;
  case NSF33FunctionKey:
    return GLFW_FKEY_F33;
  case NSF34FunctionKey:
    return GLFW_FKEY_F34;
  case NSF35FunctionKey:
    return GLFW_FKEY_F35;
...
static bool is_surrogate(UniChar uc) { return (uc - 0xd800u) < 2048u; }

static uint32_t get_first_codepoint(UniChar *utf16, UniCharCount num) {
  if (!num)
    return 0;
  if (!is_surrogate(*utf16))
    return *utf16;
  if (CFStringIsSurrogateHighCharacter(*utf16) && num > 1 &&
      CFStringIsSurrogateLowCharacter(utf16[1]))
    return CFStringGetLongCharacterForSurrogatePair(utf16[0], utf16[1]);
  return 0;
}

static bool is_pua_char(uint32_t ch) {
  return (0xE000 <= ch && ch <= 0xF8FF) || (0xF0000 <= ch && ch <= 0xFFFFF) ||
...
uint32_t vk_to_unicode_key_with_current_layout(uint16_t keycode) {
  ...
  if (UCKeyTranslate(
          [(NSData *)_glfw.ns.unicodeData bytes], keycode, kUCKeyActionDisplay,
          0, LMGetKbdType(), kUCKeyTranslateNoDeadKeysBit, &dead_key_state,
          arraysz(characters), &character_count, characters) == noErr) {
    uint32_t cp = get_first_codepoint(characters, character_count);
    if (cp) {
      if (cp < 32 || (0xF700 <= cp && cp <= 0xF8FF))
        return mac_ucode_to_functional(cp);
      if (cp >= 32 && !is_pua_char(cp))
        return cp;
    }
  }
...
static void requestRenderFrame(_GLFWwindow *w,
                               GLFWcocoarenderframefun callback) {
  if (!callback) {
...
static void updateCursorMode(_GLFWwindow *window) {
  if (window->cursorMode == GLFW_CURSOR_DISABLED) {
    ...
    _glfwPlatformGetCursorPos(window, &_glfw.ns.restoreCursorPosX,
...
static const char *format_mods(int mods) {
  static char buf[128];
  char *p = buf, *s;
...
static const char *format_text(const char *src) {
  static char buf[256];
  char *p = buf;
  ...
  while (*src) {
    int num =
        snprintf(p, sizeof(buf) - (p - buf), "0x%x ", (unsigned char)*(src++));
    if (num < 0)
      return "<error>";
    if (p + num >= last_char)
      break;
...
static const char *safe_name_for_keycode(unsigned int keycode) {
  const char *ans = _glfwPlatformGetNativeKeyName(keycode);
  if (!ans)
    return "<noname>";
  if ((1 <= ans[0] && ans[0] <= 31) || ans[0] == 127)
    ans = "<cc>";
  return ans;
}
...
//
static uint32_t translateKey(uint16_t vk_key, bool apply_keymap) {
  if (apply_keymap)
    return vk_to_unicode_key_with_current_layout(vk_key);
  uint32_t ans = vk_code_to_functional_key_code(vk_key);
  if (!ans)
    ans = vk_code_to_unicode(vk_key);
  return ans;
}
...
- (void)windowDidResize:(NSNotification *)notification {
  ...
  if (fbRect.size.width != window->ns.fbWidth ||
    ...
    _glfwInputFramebufferSize(window, (int)fbRect.size.width,
...
- (void)windowDidChangeOcclusionState:(NSNotification *)notification {
  (void)notification;
  _glfwInputWindowOcclusion(window, !([window->ns.object occlusionState] &
...
- (void)windowDidBecomeKey:(NSNotification *)notification {
  ...
  // macOS will send a delayed event to update the cursor to arrow after
  // switching desktops. So we need to delay and update the cursor once after
  // that.
  [self performSelector:@selector(request_delayed_cursor_update:)
             withObject:nil
             afterDelay:0.3];
}
...
- (void)windowDidResignKey:(NSNotification *)notification {
  ...
  if ([window->ns.view hasMarkedText]) {
    ...
    GLFWkeyevent dummy = {.action = GLFW_RELEASE,
                          .ime_state = GLFW_IME_PREEDIT_CHANGED};
...
- (void)request_delayed_cursor_update:(id)sender {
  (void)sender;
  if (window)
    window->ns.delayed_cursor_update_requested = true;
}

- (void)windowWillEnterFullScreen:(NSNotification *)notification {
  (void)notification;
  if (window)
    window->ns.in_fullscreen_transition = true;
}
...
static UInt32 convert_cocoa_to_carbon_modifiers(NSUInteger flags) {
  UInt32 mods = 0;
  if (flags & NSEventModifierFlagShift)
    mods |= shiftKey;
  if (flags & NSEventModifierFlagControl)
...
static void convert_utf16_to_utf8(UniChar *src, UniCharCount src_length,
                                  char *dest, size_t dest_sz) {
  CFStringRef string = CFStringCreateWithCharactersNoCopy(
      kCFAllocatorDefault, src, src_length, kCFAllocatorNull);
  CFStringGetCString(string, dest, dest_sz, kCFStringEncodingUTF8);
  CFRelease(string);
}

static bool alternate_key_is_ok(uint32_t key, uint32_t akey) {
  return akey > 31 && akey != key && !is_pua_char(akey);
}

static void add_alternate_keys(GLFWkeyevent *ev, NSEvent *event) {
  ev->alternate_key = translateKey(ev->native_key, false);
  if (!alternate_key_is_ok(ev->key, ev->alternate_key))
    ev->alternate_key = 0;
  if (ev->mods & GLFW_MOD_SHIFT) {
    ...
    if (ci) {
      ...
      if (sz > 0) {
        ...
        if (sz > 1)
          buf[1] = [ci characterAtIndex:1];
        ev->shifted_key = get_first_codepoint(buf, sz);
      }
    }
    if (!alternate_key_is_ok(ev->key, ev->shifted_key))
      ev->shifted_key = 0;
  }
}

static bool is_ascii_control_char(char x) {
...
#define UPDATE_PRE_EDIT_TEXT                                                   \
  ...
  if (input_context &&
      (!input_source_at_last_key_event ||
       ![input_source_at_last_key_event
           isEqualToString:input_context.selectedKeyboardInputSource])) {
    if (input_source_at_last_key_event) {
      debug_key("Input source changed, clearing pre-edit text and resetting "
                "deadkey state\n");
      GLFWkeyevent dummy = {.action = GLFW_RELEASE,
                            .ime_state = GLFW_IME_PREEDIT_CHANGED};
    ...
    input_source_at_last_key_event =
        [input_context.selectedKeyboardInputSource retain];
    [self unmarkText];
  }
  ...
  const bool process_text =
      !_glfw.ignoreOSKeyboardProcessing &&
      (!window->ns.textInputFilterCallback ||
       window->ns.textInputFilterCallback(key, mods, keycode, flags) != 1);
  _glfw.ns.text[0] = 0;
  if (keycode == 0x33 /* backspace */ || keycode == 0x35 /* escape */)
    [self unmarkText];
  GLFWkeyevent glfw_keyevent = {.key = key,
                                .native_key = keycode,
                                .native_key_id = keycode,
                                .action = GLFW_PRESS,
                                .mods = mods};
  if (!_glfw.ns.unicodeData) {
    ...
    if (UCKeyTranslate(
            [(NSData *)_glfw.ns.unicodeData bytes], keycode, kUCKeyActionDown,
            convert_cocoa_to_carbon_modifiers(flags), LMGetKbdType(),
            (process_text ? 0 : kUCKeyTranslateNoDeadKeysMask),
            &(window->ns.deadKeyState), sizeof(text) / sizeof(text[0]),
            &char_count, text) != noErr) {
      debug_key("UCKeyTranslate failed for keycode: 0x%x (%s) %s\n", keycode,
                safe_name_for_keycode(keycode), format_mods(mods));
      window->ns.deadKeyState = 0;
      return;
    }
    debug_key("\x1b[31mPress:\x1b[m native_key: 0x%x (%s) glfw_key: 0x%x "
              "%schar_count: %lu deadKeyState: %u repeat: %d ",
              keycode, safe_name_for_keycode(keycode), key, format_mods(mods),
              char_count, window->ns.deadKeyState, event.ARepeat);
    ...
    if (window->ns.deadKeyState && (char_count == 0 || keycode == 0x75)) {
      // 0x75 is the delete key which needs to be ignored during a compose
      // sequence
      debug_key(
          "Sending pre-edit text for dead key (text: %s markedText: %s).\n",
  ...
  if (!window->ns.deadKeyState) {
    ...
    if (([self hasMarkedText] || previous_has_marked_text) &&
        !_glfw.ns.text[0]) {
      // do not pass keys like BACKSPACE while there's pre-edit text, let IME
      // handle it
      debug_key(
...
static bool is_modifier_pressed(NSUInteger flags, NSUInteger target_mask,
                                NSUInteger other_mask, NSUInteger either_mask) {
...
- (void)flagsChanged:(NSEvent *)event {
  ...
  _glfw.ns.text[0] = 0;
  const NSUInteger modifierFlags =
  ...
  const bool process_text = !_glfw.ignoreOSKeyboardProcessing &&
                            (!window->ns.textInputFilterCallback ||
                             window->ns.textInputFilterCallback(
                                 key, mods, keycode, modifierFlags) != 1);
  const char *mod_name = "unknown";

  // Code for handling modifier key events copied form SDL_cocoakeyboard.m, with
  // thanks. See IsModifierKeyPressedFunction()
#define action_for(modname, target_mask, other_mask, either_mask)              \
  ...
  case GLFW_FKEY_CAPS_LOCK:
    mod_name = "capslock";
    action =
        modifierFlags & NSEventModifierFlagCapsLock ? GLFW_PRESS : GLFW_RELEASE;
    break;
  case GLFW_FKEY_LEFT_SUPER:
    action_for(super, NX_DEVICELCMDKEYMASK, NX_DEVICERCMDKEYMASK,
               NX_COMMANDMASK);
  case GLFW_FKEY_RIGHT_SUPER:
    action_for(super, NX_DEVICERCMDKEYMASK, NX_DEVICELCMDKEYMASK,
               NX_COMMANDMASK);
  case GLFW_FKEY_LEFT_CONTROL:
    action_for(ctrl, NX_DEVICELCTLKEYMASK, NX_DEVICERCTLKEYMASK,
               NX_CONTROLMASK);
  case GLFW_FKEY_RIGHT_CONTROL:
    action_for(ctrl, NX_DEVICERCTLKEYMASK, NX_DEVICELCTLKEYMASK,
               NX_CONTROLMASK);
  case GLFW_FKEY_LEFT_ALT:
    action_for(alt, NX_DEVICELALTKEYMASK, NX_DEVICERALTKEYMASK,
               NX_ALTERNATEMASK);
  case GLFW_FKEY_RIGHT_ALT:
    action_for(alt, NX_DEVICERALTKEYMASK, NX_DEVICELALTKEYMASK,
               NX_ALTERNATEMASK);
  case GLFW_FKEY_LEFT_SHIFT:
    action_for(shift, NX_DEVICELSHIFTKEYMASK, NX_DEVICERSHIFTKEYMASK,
               NX_SHIFTMASK);
  case GLFW_FKEY_RIGHT_SHIFT:
    action_for(shift, NX_DEVICERSHIFTKEYMASK, NX_DEVICELSHIFTKEYMASK,
               NX_SHIFTMASK);
  default:
    return;
  }
#undef action_for
  GLFWkeyevent glfw_keyevent = {.key = key,
                                ...
                                .action = action,
                                .mods = mods};
  debug_key("\x1b[33mflagsChanged:\x1b[m modifier: %s native_key: 0x%x (%s) "
            "glfw_key: 0x%x %s\n",
            mod_name, keycode, safe_name_for_keycode(keycode), key,
            format_mods(mods));
  marked_text_cleared_by_insert = false;
  if (process_text && input_context) {
    ...
    in_key_handler = 0;
    if (marked_text_cleared_by_insert) {
      debug_key("Clearing pre-edit text because insertText called from "
                "flagsChanged\n");
      CLEAR_PRE_EDIT_TEXT;
      if (_glfw.ns.text[0])
        glfw_keyevent.text = _glfw.ns.text;
      else
        _glfw.ns.text[0] = old_first_char;
    }
  }
  glfw_keyevent.ime_state = GLFW_IME_NONE;
  _glfwInputKeyboard(window, &glfw_keyevent);
...
- (BOOL)performDragOperation:(id<NSDraggingInfo>)sender {
  ...
  NSArray *objs =
      [pasteboard readObjectsForClasses:@[ [NSURL class], [NSString class] ]
                                options:options];
  if (!objs)
    return NO;
  const NSUInteger count = [objs count];
  NSMutableString *uri_list =
      [NSMutableString stringWithCapacity:4096]; // auto-released
  if (count) {
  ...
  if ([uri_list length] > 0)
    _glfwInputDrop(window, "text/uri-list", uri_list.UTF8String,
                   strlen(uri_list.UTF8String));
...
- (void)setMarkedText:(id)string
  ...
  debug_key("\n\tsetMarkedText: %s selectedRange: (%lu, %lu) replacementRange: "
            "(%lu, %lu)\n",
            s, selectedRange.location, selectedRange.length,
  ...
  if (!in_key_handler || in_key_handler == 2) {
    debug_key(
        "Updating IME text in kitty from setMarkedText called from %s: %s\n",
        in_key_handler ? "flagsChanged" : "event loop", _glfw.ns.text);
    GLFWkeyevent glfw_keyevent = {.text = [[markedText string] UTF8String],
                                  .ime_state = GLFW_IME_PREEDIT_CHANGED};
    _glfwInputKeyboard(window, &glfw_keyevent);
    _glfw.ns.text[0] = 0;
  }
...
- (void)updateIMEStateFor:(GLFWIMEUpdateType)which focused:(bool)focused {
  if (which == GLFW_IME_UPDATE_FOCUS && !focused && [self hasMarkedText] &&
      window) {
  ...
  if (which != GLFW_IME_UPDATE_CURSOR_POSITION)
    return;
...
- (NSRect)firstRectForCharacterRange:(NSRange)range
  ...
  if (_glfw.callbacks.get_ime_cursor_position) {
    ...
    if (window &&
        _glfw.callbacks.get_ime_cursor_position((GLFWwindow *)window, &ev)) {
      ...
      const NSRect rectInView = NSMakeRect(
          left, frame.size.height - top - cellHeight, cellWidth, cellHeight);
...
- (void)insertText:(id)string replacementRange:(NSRange)replacementRange {
  ...
  if (available_length >= required_length) {
    memcpy(_glfw.ns.text + existing_length, utf8,
           required_length); // copies the null terminator from utf8 as well
...
- (id)validRequestorForSendType:(NSString *)sendType
  ...
  if ((!sendType || [sendType isEqual:NSPasteboardTypeString] ||
       [sendType isEqual:@"NSStringPboardType"]) &&
      (!returnType || [returnType isEqual:NSPasteboardTypeString] ||
       [returnType isEqual:@"NSStringPboardType"])) {
    if (_glfw.callbacks.has_current_selection &&
        _glfw.callbacks.has_current_selection())
      return self;
  }
...
// For example, after selecting an absolute path, open the global menu bar
// kitty->Services and click `Show in Finder`.
...
// For example, open System Settings->Keyboard->Keyboard
// Shortcuts->Services->Text, enable `Convert Text to Full Width`, select some
// text and execute the service.
- (BOOL)readSelectionFromPasteboard:(NSPasteboard *)pboard {
  NSString *text = nil;
  NSArray *types = [pboard types];
  if ([types containsObject:NSPasteboardTypeString] == YES) {
    text =
        [pboard stringForType:NSPasteboardTypeString]; // public.utf8-plain-text
  } else if ([types containsObject:@"NSStringPboardType"] == YES) {
    text = [pboard stringForType:@"NSStringPboardType"]; // for older services
                                                         // (need re-encode?)
  } else {
    return NO;
  }
  if (text && [text length] > 0) {
    // The service wants us to replace the selection, but we can't replace
    // anything but insert text.
...
- (void)performMiniaturize:(id)sender {
  if (glfw_window &&
      (!glfw_window->decorated || glfw_window->ns.titlebar_hidden))
...
- (BOOL)canBecomeMainWindow {
  return !glfw_window->ns.layer_shell.is_active ||
...
- (void)toggleFullScreen:(nullable id)sender {
  if (glfw_window) {
    if (glfw_window->ns.in_fullscreen_transition)
      return;
    if (glfw_window->ns.toggleFullscreenCallback &&
        glfw_window->ns.toggleFullscreenCallback((GLFWwindow *)glfw_window) ==
            1)
      return;
  ...
  // When resizeIncrements is set, Cocoa cannot restore the original window size
  // after returning from fullscreen.
  const NSSize original = [self resizeIncrements];
  [self setResizeIncrements:NSMakeSize(1.0, 1.0)];
  [super toggleFullScreen:sender];
  [self setResizeIncrements:original];
  // When the window decoration is hidden, toggling fullscreen causes the style
  // mask to be changed, and causes the first responder to be cleared.
  if (glfw_window && !glfw_window->decorated && glfw_window->ns.view)
    [self makeFirstResponder:glfw_window->ns.view];
  update_titlebar_button_visibility_after_fullscreen_transition(
      glfw_window, false, !is_fullscreen_already);
}
...
static bool createNativeWindow(_GLFWwindow *window,
  ...
  window->ns.object =
      [[GLFWWindow alloc] initWithGlfwWindow:contentRect
...
int _glfwPlatformCreateWindow(_GLFWwindow *window,
  ...
  if (lsc) {
    ...
    window->ns.layer_shell.is_active = false;
  ...
  if (window->monitor) {
    // Do not show the window here until after setting the window size,
    // maximized state, and full screen _glfwPlatformShowWindow(window);
...
static NSScreen *screen_for_name(const char *name) {
  int count = 0;
  GLFWmonitor **monitors = glfwGetMonitors(&count);
  for (int i = 0; i < count; i++) {
    const char *q = glfwGetMonitorName(monitors[i]);
    if (q && strcmp(q, name) == 0)
      return ((_GLFWmonitor *)monitors[i])->ns.screen;
  }
  return NULL;
}

bool _glfwPlatformSetLayerShellConfig(_GLFWwindow *window,
                                      const GLFWLayerShellConfig *value) {
...
#define nswindow window->ns.object
  ...
  if (!is_transparent || config.related.background_opacity >= 1.f) {
    background_blur = 0;
  }
  ...
  case SRGB_COLORSPACE:
    ...
    break;
  case DISPLAY_P3_COLORSPACE:
    cs = [NSColorSpace displayP3ColorSpace];
    break;
  case DEFAULT_COLORSPACE:
    cs = nil;
    break; // using deviceRGBColorSpace causes a hang when transitioning to
           // fullscreen
  }
  ...
  double left_edge_spacing, top_edge_spacing, right_edge_spacing,
      bottom_edge_spacing;
  float xscale = (float)config.expected.xscale,
        yscale = (float)config.expected.yscale;
  _glfwPlatformGetWindowContentScale(window, &xscale, &yscale);
  config.size_callback((GLFWwindow *)window, xscale, yscale, &cell_width,
                       &cell_height, &left_edge_spacing, &top_edge_spacing,
                       &right_edge_spacing, &bottom_edge_spacing);
  ...
  const unsigned xsz = config.x_size_in_pixels
                           ? (unsigned)(config.x_size_in_pixels * xscale)
                           : (cell_width * config.x_size_in_cells);
  const unsigned ysz = config.y_size_in_pixels
                           ? (unsigned)(config.y_size_in_pixels * yscale)
                           : (cell_height * config.y_size_in_cells);
  CGFloat dock_height = NSMinY(screen.visibleFrame) - NSMinY(screen.frame);
  CGFloat menubar_height =
      NSHeight(screen.frame) - NSHeight(screen.visibleFrame) - dock_height;
  CGFloat x = NSMinX(screen.visibleFrame), y = NSMinY(screen.visibleFrame) - 1,
          width = NSWidth(screen.visibleFrame),
          height = NSHeight(screen.visibleFrame) + 2;
  if (config.type == GLFW_LAYER_SHELL_BACKGROUND ||
      config.edge == GLFW_EDGE_CENTER) {
    x = NSMinX(screen.frame);
    height = NSHeight(screen.frame) - menubar_height + 1;
    y = NSMinY(screen.frame);
    width = NSWidth(screen.frame);
  }
  // Screen co-ordinate system is with origin in lower left and y increasing
  // upwards and x increasing rightwards NSLog(@"frame: %@ visibleFrame: %@\n",
  // NSStringFromRect(screen.frame), NSStringFromRect(screen.visibleFrame));
  NSWindowLevel level = NSPopUpMenuWindowLevel -
                        1; // so that popup menus from globalmenubar function
  NSWindowAnimationBehavior animation_behavior =
      NSWindowAnimationBehaviorUtilityWindow;
  switch (config.type) {
  case GLFW_LAYER_SHELL_BACKGROUND:
    animation_behavior = NSWindowAnimationBehaviorNone;
    // See:
    // https://stackoverflow.com/questions/4982584/how-do-i-draw-the-desktop-on-mac-os-x/4982619#4982619
    level = kCGDesktopWindowLevel;
    break;
  case GLFW_LAYER_SHELL_OVERLAY:
  case GLFW_LAYER_SHELL_NONE:
    break;
  ...
  if (config.type != GLFW_LAYER_SHELL_BACKGROUND &&
      config.edge != GLFW_EDGE_CENTER) {
    double panel_height = spacing_y + ysz / yscale,
           panel_width = spacing_x + xsz / xscale;
    switch (config.edge) {
...
void _glfwPlatformSetWindowTitle(_GLFWwindow *window, const char *title) {
  ...
  if (!string)
    return; // the runtime failed to convert title to an NSString
...
void _glfwPlatformGetWindowPos(_GLFWwindow *window, int *xpos, int *ypos) {
  ...
  if (ypos)
    *ypos = (int)_glfwTransformYNS(contentRect.origin.y +
...
void _glfwPlatformSetWindowSize(_GLFWwindow *window, int width, int height) {
  ...
  if (window->monitor) {
    ...
    if ([window->ns.object styleMask] & NSWindowStyleMaskFullScreen ||
...
void _glfwPlatformSetWindowSizeIncrements(_GLFWwindow *window, int widthincr,
                                          int heightincr) {
  if (widthincr != GLFW_DONT_CARE && heightincr != GLFW_DONT_CARE) {
    float xscale = 1, yscale = 1;
    _glfwPlatformGetWindowContentScale(window, &xscale, &yscale);
    [window->ns.object setResizeIncrements:NSMakeSize(widthincr / xscale,
                                                      heightincr / yscale)];
  } else {
    [window->ns.object setResizeIncrements:NSMakeSize(1.0, 1.0)];
  }
...
void _glfwPlatformSetRawMouseMotion(_GLFWwindow *window UNUSED,
                                    bool enabled UNUSED) {
...
void _glfwPlatformSetCursorPos(_GLFWwindow *window, double x, double y) {
  ...
  if (window->monitor) {
    ...
    CGWarpMouseCursorPosition(
        CGPointMake(globalPoint.x, _glfwTransformYNS(globalPoint.y)));
  }
}
...
const char *_glfwPlatformGetNativeKeyName(int keycode) {
  ...
  if (UCKeyTranslate([(NSData *)_glfw.ns.unicodeData bytes], keycode,
  ...
  convert_utf16_to_utf8(characters, characterCount, _glfw.ns.keyName,
...
int _glfwPlatformGetNativeKeyForKey(uint32_t glfw_key) {
  if (GLFW_FKEY_FIRST <= glfw_key && glfw_key <= GLFW_FKEY_LAST) { // {{{
    switch (glfw_key) {
      /* start functional to macu (auto generated by gen-key-constants.py do not
       * edit) */
...
static NSCursor *load_hidden_system_cursor(NSString *name, SEL fallback) {
  ...
  NSImage *image = [[NSImage alloc]
      initWithContentsOfFile:[cursorPath
                                 stringByAppendingPathComponent:@"cursor.pdf"]];
  ...
  if (frames > 1) {
    ...
    const NSSize cropped_size =
        NSMakeSize(image.size.width, (int)(image.size.height / frames));
    ...
    [cropped lockFocus];
    {
      const NSRect cropped_rect =
          NSMakeRect(0, 0, cropped_size.width, cropped_size.height);
      [image drawInRect:cropped_rect
               fromRect:cropped_rect
              operation:operation
               fraction:1];
    }
    [cropped unlockFocus];
...
#define C(name, val)                                                           \
  case name:                                                                   \
    cursor->ns.object = [NSCursor val];                                        \
    break;
#define U(name, val)                                                           \
  case name:                                                                   \
    cursor->ns.object = [NSCursor performSelector:@selector(val)];             \
    break;
#define S(name, val, fallback)                                                 \
  case name:                                                                   \
    cursor->ns.object = load_hidden_system_cursor(@ #val, @selector(val));     \
    break;
  switch (shape) {
    ...
    S(GLFW_NE_RESIZE_CURSOR, resizenortheast,
      _windowResizeNorthEastSouthWestCursor);
    S(GLFW_NW_RESIZE_CURSOR, resizenorthwest,
      _windowResizeNorthWestSouthEastCursor);
    C(GLFW_N_RESIZE_CURSOR, resizeUpCursor);
    S(GLFW_SE_RESIZE_CURSOR, resizesoutheast,
      _windowResizeNorthWestSouthEastCursor);
    S(GLFW_SW_RESIZE_CURSOR, resizesouthwest,
      _windowResizeNorthEastSouthWestCursor);
...
bool _glfwPlatformIsFullscreen(_GLFWwindow *w, unsigned int flags) {
  ...
  if (traditional) {
    if (@available(macOS 10.15.7, *))
      return w->ns.in_traditional_fullscreen;
  }
  NSWindowStyleMask sm = [window styleMask];
  return sm & NSWindowStyleMaskFullScreen;
}

static void make_window_fullscreen_after_show(unsigned long long timer_id,
                                              void *data) {
  (void)timer_id;
  ...
  for (_GLFWwindow *w = _glfw.windowListHead; w; w = w->next) {
    if (w->id == window_id) {
      ...
      update_titlebar_button_visibility_after_fullscreen_transition(w, false,
                                                                    true);
      break;
    }
  }
...
bool _glfwPlatformToggleFullscreen(_GLFWwindow *w, unsigned int flags) {
  ...
  if (traditional) {
    ...
    // If the dock and menubar are hidden going from maximized to fullscreen
    // doesn't change the window size and macOS forgets to trigger
    // windowDidResize, so call it ourselves
    NSNotification *notification =
        [NSNotification notificationWithName:NSWindowDidResizeNotification
                                      object:window];
    [w->ns.delegate performSelector:@selector(windowDidResize:)
    ...
    if (!in_fullscreen && !_glfwPlatformWindowVisible(w)) {
      ...
      // is already fullscreen. Le sigh.
      // https://github.com/kovidgoyal/kitty/issues/7448
      _glfwPlatformAddTimer(0, false, make_window_fullscreen_after_show,
                            (void *)(uintptr_t)(w->id), NULL);
...
static NSString *mime_to_uti(const char *mime) {
...
static const char *uti_to_mime(NSString *uti) {
  if ([uti isEqualToString:NSPasteboardTypeString])
    return "text/plain";
  if ([uti hasPrefix:UTI_ROUNDTRIP_PREFIX])
    return [[uti substringFromIndex:[UTI_ROUNDTRIP_PREFIX length]] UTF8String];
  if (@available(macOS 11.0, *)) {
    UTType *t = [UTType typeWithIdentifier:uti]; // auto-released
    if (t.preferredMIMEType != nil)
      return [t.preferredMIMEType UTF8String];
  }
  return "";
}

static void list_clipboard_mimetypes(GLFWclipboardwritedatafun write_data,
                                     void *object) {
#define w(x)                                                                   \
  {                                                                            \
    if (ok)                                                                    \
      ok = write_data(object, x, strlen(x));                                   \
  }
  NSPasteboard *pasteboard = [NSPasteboard generalPasteboard];
  NSDictionary *options = @{NSPasteboardURLReadingFileURLsOnlyKey : @YES};
  BOOL has_file_urls = [pasteboard canReadObjectForClasses:@[ [NSURL class] ]
                                                   options:options];
  BOOL has_strings = [pasteboard canReadObjectForClasses:@[ [NSString class] ]
                                                 options:nil];
...
static void get_text_plain(GLFWclipboardwritedatafun write_data, void *object) {
  NSPasteboard *pasteboard = [NSPasteboard generalPasteboard];
  NSDictionary *options = @{NSPasteboardURLReadingFileURLsOnlyKey : @YES};
  NSArray *objs =
      [pasteboard readObjectsForClasses:@[ [NSURL class], [NSString class] ]
  ...
  if (objs) {
    const NSUInteger count = [objs count];
    if (count) {
      NSMutableData *path_list =
          [NSMutableData dataWithCapacity:4096]; // auto-released
      NSMutableData *text_list =
          [NSMutableData dataWithCapacity:4096]; // auto-released
      for (NSUInteger i = 0; i < count; i++) {
        id obj = objs[i];
        if ([obj isKindOfClass:[NSURL class]]) {
          ...
          if (url.fileURL && url.fileSystemRepresentation) {
            if ([path_list length] > 0)
              [path_list appendBytes:"\n" length:1];
            [path_list appendBytes:url.fileSystemRepresentation
                            length:strlen(url.fileSystemRepresentation)];
          }
        } else if ([obj isKindOfClass:[NSString class]]) {
          if ([text_list length] > 0)
            [text_list appendBytes:"\n" length:1];
  ...
  if (!found)
    _glfwInputError(GLFW_PLATFORM_ERROR,
                    "Cocoa: Failed to retrieve text/plain from pasteboard");
}

void _glfwPlatformGetClipboard(GLFWClipboardType clipboard_type,
                               const char *mime_type,
                               GLFWclipboardwritedatafun write_data,
                               void *object) {
  if (clipboard_type != GLFW_CLIPBOARD)
    return;
  if (mime_type == NULL) {
  ...
  NSPasteboardType t =
      [pasteboard availableTypeFromArray:@[ mime_to_uti(mime_type) ]];
...
static NSMutableData *get_clipboard_data(const _GLFWClipboardData *cd,
                                         const char *mime) {
  ...
  while (true) {
    ...
    if (chunk.free)
      chunk.free((void *)chunk.free_data);
  }
...
void _glfwPlatformSetClipboard(GLFWClipboardType t) {
  if (t != GLFW_CLIPBOARD)
    return;
  NSPasteboard *pasteboard = [NSPasteboard generalPasteboard];
  NSMutableArray<NSPasteboardType> *ptypes = [NSMutableArray
      arrayWithCapacity:_glfw.clipboard.num_mime_types]; // auto-released
  ...
  for (size_t i = 0; i < _glfw.clipboard.num_mime_types; i++) {
    NSMutableData *data = get_clipboard_data(
        &_glfw.clipboard, _glfw.clipboard.mime_types[i]); // auto-released
    /* NSLog(@"putting data: %@ for: %s with UTI: %@", data,
     * _glfw.clipboard.mime_types[i], ptypes[i]); */
    if (data != nil)
      [pasteboard setData:data forType:ptypes[i]];
  }
}
// }}}
...
int _glfwPlatformGetPhysicalDevicePresentationSupport(
    VkInstance instance UNUSED, VkPhysicalDevice device UNUSED,
...
int _glfwPlatformSetWindowBlur(_GLFWwindow *window, int radius) {
  int orig = window->ns.blur_radius;
  if (radius > -1 && radius != window->ns.blur_radius) {
    extern OSStatus CGSSetWindowBackgroundBlurRadius(
        void *connection, NSInteger windowNumber, int radius);
    extern void *CGSDefaultConnectionForThread(void);
    CGSSetWindowBackgroundBlurRadius(CGSDefaultConnectionForThread(),
                                     [window->ns.object windowNumber], radius);
...
GLFWAPI GLFWcocoatextinputfilterfun glfwSetCocoaTextInputFilter(
    GLFWwindow *handle, GLFWcocoatextinputfilterfun callback) {
  _GLFWwindow *window = (_GLFWwindow *)handle;
  _GLFW_REQUIRE_INIT_OR_RETURN(nil);
...
GLFWAPI GLFWcocoatogglefullscreenfun glfwSetCocoaToggleFullscreenIntercept(
    GLFWwindow *handle, GLFWcocoatogglefullscreenfun callback) {
  _GLFWwindow *window = (_GLFWwindow *)handle;
  _GLFW_REQUIRE_INIT_OR_RETURN(nil);
...
GLFWAPI void glfwCocoaRequestRenderFrame(GLFWwindow *w,
                                         GLFWcocoarenderframefun callback) {
...
GLFWAPI void glfwCocoaSetWindowChrome(
  ...
  @autoreleasepool {
    ...
    if (use_system_color || background_opacity < 1.0) {
      if (is_transparent) {
        // prevent blurring of shadows at window corners with desktop background
        // by setting a low alpha background
        background = background_blur > 0
      ...
      // set a background color and make the title bar transparent so the
      ...
      background = [NSColor colorWithSRGBRed:red
                                       green:green
                                        blue:blue
                                       alpha:1.f];
    ...
    [window->ns.object setTitleVisibility:(show_text_in_titlebar)
    ...
    debug("Window Chrome state:\n\tbackground: %s\n\tappearance: %s "
          ...
          "titlebar: transparent: %d title_visibility: %d hidden: %d "
          "buttons_hidden: %d"
          "\n",
          ...
          window->decorated, titlebar_transparent, show_text_in_titlebar,
          window->ns.titlebar_hidden, hide_titlebar_buttons);
    ...
    if (in_fullscreen && window->ns.in_traditional_fullscreen) {
      ...
      [window->ns.object
          setStyleMask:window->ns.pre_full_screen_style_mask | fsmask];
    }
    // HACK: Changing the style mask can cause the first responder to be cleared
    [window->ns.object makeFirstResponder:window->ns.view];
  }
...
glfwGetCurrentSystemColorTheme(bool query_if_unintialized) {
  ...
  NSAppearanceName newAppearance =
      [changedAppearance bestMatchFromAppearancesWithNames:@[
